...
Pde2::Pde2(const ST dt, const ST dx,const int test_no,const int numThreads):dt(dt),dirac_width(dx) {
...
void Pde2::add_particle(const ST x, const ST y, const ST z) {
	typedef Teuchos::ScalarTraits<ST> STS;
	typedef STS::magnitudeType MT;
	typedef Teuchos::ScalarTraits<MT> STM;
...
void Pde2::setup_pamgen_mesh(const std::string& meshInput){
...
void Pde2::create_cubature_and_basis() {
	using namespace Intrepid;
	/**********************************************************************************/
...
void Pde2::build_maps_and_create_matrices() {
...
void Pde2::integrate(const ST requested_dt) {
...
void Pde2::materialTensor (Scalar material[][3], const Scalar& x, const Scalar& y,const Scalar& z) {
...
void Pde2::evaluateMaterialTensor (ArrayOut& matTensorValues, const ArrayIn& evaluationPoints) {
...
std::string Pde2::makeMeshInput (const int nx, const int ny, const int nz) {
  using std::endl;
...
std::string Pde2::makeMeshInputFullDomain (const int nx, const int ny, const int nz) {
  using std::endl;
  std::ostringstream os;
...
std::string Pde2::makeMeshInputSphere (const int nr, const int ntheta) {
  using std::endl;
  std::ostringstream os;
...
std::string Pde2::makeMeshInputRadialTrisection (const int nr, const int ntheta, const int nz) {
  using std::endl;
  std::ostringstream os;
...
std::string Pde2::makeMeshInputCylinder (const int nr, const int ntheta, const int nz) {
  using std::endl;
  std::ostringstream os;
...
vtkUnstructuredGrid* Pde2::get_grid() {
	return vtk_grid;
}

vtkUnstructuredGrid* Pde2::get_boundary() {
	return vtk_boundary;
}

void Pde2::create_vtk_grid() {
	//TODO: assumes 1 cpu
	/*
	 * setup points
	 */
	...
	outflow_scalar->SetArray(U->getDataNonConst(0).getRawPtr(),num_overlapped_boundary_points,1);
...
RCP<Pde2::vector_type> Pde2::get_boundary_node_values() {
	return Lambda;
}

RCP<Pde2::vector_type> Pde2::get_boundary_node_areas() {
	return boundary_node_areas;
}

RCP<Pde2::multivector_type> Pde2::get_boundary_node_positions() {
	return boundary_node_positions;
}

int Pde2::get_total_number_of_particles() {
...
void Pde2::calculate_volumes_and_areas() {
	using Tpetra::RTI::reduce;
	using Tpetra::RTI::ZeroOp;
	using Tpetra::RTI::reductionGlob;
...
void Pde2::rescale(double s) {
	U->scale(s);
}


void Pde2::boundary_integrals() {
	using namespace Intrepid;
	using Teuchos::ArrayView;
	using Teuchos::arrayView;
	using Teuchos::as;
...
void Pde2::compose_LHS_and_RHS() {
	  ...
	  //strategy.paramsXmlFileName("paramsFile.xml");
...
//	  Teuchos::Array<RCP<VectorSpaceBase<ST> > > U_Lambda;
//	  U_Lambda.push_back(U_w);
//	  U_Lambda.push_back(Lambda_w);
//	  X = productVectorSpace<ST>(U_Lambda())->createMember();
//
//	  RCP< VectorBase<ST> > U_rhs_w = createVector(U_rhs);
//	  RCP< VectorBase<ST> > Lambda_rhs_w = createVector(Lambda_rhs);
//	  Teuchos::Array<RCP<VectorSpaceBase<ST> > > U_Lambda_rhs;
//	  U_Lambda_rhs.push_back(U_rhs_w);
//	  U_Lambda_rhs.push_back(Lambda_rhs_w);
//	  Y = productVectorSpace<ST>(U_Lambda_rhs())->createMember();
	  ...
	  RCP<const linearOp> A = add<ST>(Mi_w,scale(omega*dt,K_w,"omega_dt_K"),"A");
	  ...
	  RHS = block2x2(
			  subtract(Mi_w,scale((1-omega*dt),K_w)), scale(omega-1,transpose(B_w,"B_t")),
	  ...
	  RCP<const linearOp> S = multiply<ST>(omegaB,invA,omegaB_t);

	  RCP<const linearOp> LHS_prec = block2x2<ST>(A,zero01,
			  	  	  	  	  	  	  	  	  	 zero10,S);

	  std::cout << Teuchos::describe(*LHS_prec,Teuchos::VERB_MEDIUM);

	  LHS = lowsFactory->createOp();
	  Thyra::initializeApproxPreconditionedOp<ST>(*lowsFactory,LHSop,LHS_prec,LHS.ptr());
